...
const CatBreedsList = () => {
    const [breedsList, setBreedsList] = useState([]); // Для хранения данных
    ...
    useEffect(() => {
        const fetchBreeds = async () => {
            try {
                const response = await axios.get("/api/cat_breeds"); // Убедитесь, что API правильно настроен
                setBreedsList(response.data); // Установить данные в состояние
                setIsLoading(false); // Отключить индикатор загрузки
            } catch (err) {
        ...
        fetchBreeds(); // Вызов функции
    }, []); // Пустой массив зависимостей, чтобы запрос выполнялся только при первом рендере
